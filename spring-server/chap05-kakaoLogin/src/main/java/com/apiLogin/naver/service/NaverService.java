package com.apiLogin.naver.service;
...
public class NaverService {
	...
	//"${naver.client.id}" : application.properties에 적혀있는 naver.client.id를 찾아서 가져온다
	...
	public NaverDTO getNaverInfo(String code) throws Exception {
		if(code == null) throw new Exception("인증되지 않았습니다.");
		String accessToken = "";
		String refreshToken = "";
	...
	private NaverDTO getUserInfoToken(String accessToken) throws Exception {
		...
		//response에서 받은 HTTP 응답 본문 Json 넣어서
		//JSONObject는 key value 형식으로 이루어짐
		JSONObject jsonObj = (JSONObject)jsonParser.parse(response.getBody());
		//JSONObject jsonObj로 가져온 key value 값에서
		//response만 찾아서 account 안에 넣어준다
